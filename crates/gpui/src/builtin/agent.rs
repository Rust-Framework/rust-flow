//! Agent 节点：智能体配置，In + Out 端口，标题栏 + 主体结构。
//!
//! 主体显示模型名称或节点类型标签。
//! 属性面板支持配置模型（model）和系统提示词（prompt）。
...
/// 从 node.data 读取模型名称。
fn model_of(node: &Node) -> Option<String> {
    node.data
        .get("model")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
            ...
            schema: NodeSchema::new("agent", "Agent")
                ...
                .with_field(
                    FieldSpec::new("label", "Label", FieldType::Text)
                        .with_default(serde_json::json!("Agent")),
                )
                .with_field(
                    FieldSpec::new("model", "Model", FieldType::Text)
                        .with_default(serde_json::json!("gpt-4")),
                )
                .with_field(
                    FieldSpec::new("prompt", "System Prompt", FieldType::TextArea)
...
impl IFlowNode for AgentNode {
    ...
    fn get_view(&self, node: &Node, ctx: &mut NodeViewCtx) -> AnyElement {
        ...
        let body_text = model_of(node).unwrap_or_else(|| crate::i18n::t(lang, TKey::Agent).to_string());
